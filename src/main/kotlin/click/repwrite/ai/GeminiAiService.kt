...
class GeminiAiService(
    ...
    init {
        ...
        val apiKey =
                geminiProperties.apiKey
                        ?: try {
                            logger.info(
                                    "Gemini API key not found in properties, attempting to retrieve from AWS Secrets Manager..."
                            )
                            ...
                            logger.warn(
                                    "Failed to retrieve Gemini API key from AWS Secrets Manager: ${e.message}"
                            )
                            null
                        }
    ...
    fun generateEmail(cause: Cause, senator: Senator, userDetails: EmailRequest): EmailResponse? {
        ...
        val prompt =
            ...
            **Output Format:**
            Return ONLY a valid JSON object with the following structure:
            {
              ...
              "body": "The full email body, 2-3 paragraphs, including salutation and sign-off",
              "tweet": "The short tweet content",
              "phoneLine": "The one-sentence phone script"
            }
        """.trimIndent()
    ...
    private fun <T> callGemini(
        ...
        if (file != null && file.exists()) {
            try {
                ...
                logger.error("Failed to read/encode file attachment for Gemini: ${e.message}", e)
            }
        }
        ...
        try {
            val response =
                    restTemplate.postForObject(uri, httpEntity, GeminiApiResponse::class.java)

            val responseJsonString =
                    response?.candidates?.firstOrNull()?.content?.parts?.firstOrNull()?.text
                            ?: throw IOException("Failed to extract response text from Gemini API")

            val cleanedJson =
                    responseJsonString.trim().removeSurrounding("```json\n", "\n```").trim()
